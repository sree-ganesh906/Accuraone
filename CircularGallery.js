...
class Media {
      ...
      fragment: `
        ...
        void main() {
          ...
          if (planeRatio > imageRatio) {
              // Plane is wider than image. Fit to height.
              scale.x = imageRatio / planeRatio;
          } else {
              // Plane is taller than image. Fit to width.
              scale.y = planeRatio / imageRatio;
          }
          
          vec2 uv = (vUv - 0.5) / scale + 0.5;
          
          vec4 color = vec4(1.0); // White background
          if (uv.x >= 0.0 && uv.x <= 1.0 && uv.y >= 0.0 && uv.y <= 1.0) {
              vec4 texColor = texture2D(tMap, uv);
              color = mix(vec4(1.0), texColor, texColor.a);
          }
  ...
  onResize({ screen, viewport } = {}) {
    ...
    // Determine how many items should be visible based on screen width
    let visibleItems = 5.5;
    if (this.screen.width < 1024) visibleItems = 4.5;
    if (this.screen.width < 768) visibleItems = 3.5;
    if (this.screen.width < 480) visibleItems = 2.5;

    this.width = this.viewport.width / visibleItems;
    this.padding = this.width * 0.15; // 15% of the allocated width is gap
    
    this.plane.scale.x = this.width - this.padding;
    this.plane.scale.y = this.plane.scale.x * 0.75; // 4:3 aspect ratio for cards
...
class CircularGalleryApp {
  ...
  addEventListeners() {
    ...
    // Bind touch events to container instead of window for better UX
