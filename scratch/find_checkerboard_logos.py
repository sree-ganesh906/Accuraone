...
# Walk through all directories and find png and jpg files
    ...
    for f in os.listdir(d):
        if f.lower().endswith(('.png', '.jpg', '.jpeg')):
            ...
            try:
                ...
                if img is None:
                    continue
                # Calculate distance to [238, 238, 238] (which is a standard gray check color in hex #eeeeee)
                dist_to_check = np.linalg.norm(img - [238, 238, 238], axis=2)
                # Count how many pixels are extremely close (distance < 2)
                check_pixels = np.sum(dist_to_check < 2)
                pct = check_pixels / (img.shape[0] * img.shape[1]) * 100
                if pct > 5.0: # If more than 5% of pixels are #eeeeee
